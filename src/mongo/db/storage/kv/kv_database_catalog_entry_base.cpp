...
void KVDatabaseCatalogEntryBase::getCollectionNamespaces(OperationContext* opCtx, std::list<std::string>* out) const {
    auto& uuidCatalog = UUIDCatalog::get(opCtx);
    for (auto it = uuidCatalog.begin(name()); it != uuidCatalog.end(); ++it) {
        auto coll = *it;
        if (!coll) {
...
CollectionCatalogEntry* KVDatabaseCatalogEntryBase::getCollectionCatalogEntry(OperationContext* opCtx, StringData ns) const {
    return UUIDCatalog::get(opCtx).lookupCollectionCatalogEntryByNamespace(nss);
}

RecordStore* KVDatabaseCatalogEntryBase::getRecordStore(OperationContext* opCtx, StringData ns) const {
...
Status KVDatabaseCatalogEntryBase::createCollection(OperationContext* opCtx,
    ...
    auto collCatalogEntry = std::make_unique<KVCollectionCatalogEntry>(_engine, _engine->getCatalog(), nss.ns(), ident, std::move(rs));
...
void KVDatabaseCatalogEntryBase::initCollection(OperationContext* opCtx,
    ...
    auto collCatalogEntry = std::make_unique<KVCollectionCatalogEntry>(_engine, _engine->getCatalog(), ns, ident, std::move(rs));
    auto uuid = *(collCatalogEntry->getUUID(opCtx));
    uuidCatalog.insertCollectionCatalogEntry(uuid, std::move(collCatalogEntry));
}
...
Status KVDatabaseCatalogEntryBase::renameCollection(OperationContext* opCtx,
    ...
    RecordStore* originalRS = checked_cast<KVCollectionCatalogEntry*>(fromCatalogEntry)->getRecordStore();
